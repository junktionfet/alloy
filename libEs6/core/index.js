...
/* JSG - Added locker concept to this module */
const locker = {};
export const getLocker = () => locker;
...
export const createExecuteCommand = ({
  ...
  /* JSG - Stash scope in module locker */
  locker.scope = scope;
...
/*
  JSG - The default export is not used in our implementation.
  The "scope" object replaces "window"
*/
export default ((scope = {}, console = window.console) => {
  // eslint-disable-next-line no-underscore-dangle
  const instanceNames = scope.__alloyNS;

  // set this up as a function so that monitors can be added at anytime
  // eslint-disable-next-line no-underscore-dangle
  const getMonitors = () => scope.__alloyMonitors || [];
  ...
  if (instanceNames) {
    instanceNames.forEach(instanceName => {
      ...
      const queue = scope[instanceName].q;
      queue.push = instance;
