...
import { isInteger } from "../../utils"; // The retry gets incrementally (but not exponentially) longer for each retry.

const FIRST_DELAY_MILLIS = 1000;
const INCREMENTAL_DELAY_MILLIS = 1000; // When the target delay is randomized, make it within the range of this percentage above or below the target delay.
...
export default (({
  ...
  let delayInMillis = getDelayFromHeader(response); // Note that the value of delay may be 0 at this point, which would be a valid delay we want to use
