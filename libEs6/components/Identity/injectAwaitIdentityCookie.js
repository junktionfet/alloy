    ...
    return new Promise((resolve, reject) => {
      onResponse(() => {
        if (doesIdentityCookieExist()) {
          ...
          const noIdentityCookieError = new Error(`An identity was not set properly. Please verify that the org ID ${orgId} configured in Alloy matches the org ID specified in the edge configuration.`); // Rejecting the promise will reject commands that were queued
